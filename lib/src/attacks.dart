...
SquareSet silverAttacks(Square square, Side side) {
  if (side == Side.sente) {
    return _neighbors[square].withoutMany([
      square + 16,
      square - 1,
      square + 1,
    ]);
...
SquareSet lionAttacks(Square square) {
  return _neighbors[square].union(
    _computeRange(square, [
      -34,
      -33,
      -32,
      -31,
...
SquareSet _computeRange(Square square, List<int> deltas) {
  ...
  final dests =
      deltas
          ...
          .where(
            (sq) => sq >= 0 && sq < 256 && (file - Square(sq).file).abs() <= 2,
          )
          .toList();
...
final _fileRange = _tabulateSquares(
  (sq) => SquareSet.fromFile(sq.file).withoutSquare(sq),
);

final _rankRange = _tabulateSquares(
  (sq) => SquareSet.fromRank(sq.rank).withoutSquare(sq),
);
...
final _diagRange = _tabulateSquares((sq) {
  ...
  return (shift >= 0 ? diag.shl256(shift) : diag.shr256(-shift)).withoutSquare(
    sq,
  );
});
