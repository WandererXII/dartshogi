...
String Function(Piece) pieceToBoardKanji(Rule rule) {
  return (piece) => piece.side == Side.gote
...
int? parseJapaneseSquare(String str) {
  ...
  final fileOffset = str.length == 2 || (str.length == 3 && str[1] == '十') ? 1 : 2;
  final file = int.tryParse(
        str.substring(0, fileOffset).split('').map((c) {
          return c.codeUnitAt(0) >= 0xfee0 + 48 ? String.fromCharCode(c.codeUnitAt(0) - 0xfee0) : c;
        }).join(),
      ) ??
      -1 - 1;
