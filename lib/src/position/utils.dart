  ...
  Piece? capture,
    ...
    switch (rule) {
      Rule.chushogi => () {
          ...
          final forcedEdgeRank = (piece.role == Role.pawn || piece.role == Role.lance) &&
      ...
      _ => promotableRoles(rule).contains(piece.role) &&
          (promotionZone(rule, piece.side).has(from) || promotionZone(rule, piece.side).has(to)),
    };
...
SquareSet promotionZone(Rule rule, Side side) => switch (rule) {
      ...
      Rule.minishogi => side == Side.sente
